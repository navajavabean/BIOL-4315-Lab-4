...
### Download *Arabidopsis thaliana* Data 
...
# Use file.path() to correctly join the directory and file names
...
Files 1-12 have reads that are between >71 - <101 cycles long Files 13-36 have reads that are primarily >71 cycles long but shorter than 81. 

The shortest number of cycles found across all your samples is 71

```{r plots,echo=TRUE, warning=FALSE, message=FALSE}
library(Rqc)
qa_report <- rqc(path = fq_path, pattern = "\\.fastq\\.gz$")

# generate per-cycle Q-score boxplots
rqcCycleQualityBoxPlot(qa_report[1:12]) + theme_bw() + ggtitle("Q-Scores: Files 1-12")
rqcCycleQualityBoxPlot(qa_report[13:24]) + theme_bw() + ggtitle("Q-Scores: Files 13-24")
rqcCycleQualityBoxPlot(qa_report[25:36]) + theme_bw() + ggtitle("Q-Scores: Files 25-36")

# generate per-cycle base call frequency plts
rqcCycleBaseCallsLinePlot(qa_report[1:12]) + ggtitle("Base Calls: Files 1-12")
rqcCycleBaseCallsLinePlot(qa_report[13:24]) + ggtitle("Base Calls: Files 13-24")
